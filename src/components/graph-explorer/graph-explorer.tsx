...
export function GraphExplorer() {
  ...
  return <div><div className="mode-tabs" role="tablist" aria-label="Graph analysis mode"><button type="button" role="tab" aria-selected={mode === "path"} className="mode-tab" data-active={mode === "path"} onClick={() => setMode("path")}><Route aria-hidden="true" size={16} />Path Finder</button><button type="button" role="tab" aria-selected={mode === "blast"} className="mode-tab" data-active={mode === "blast"} onClick={() => setMode("blast")}><GitBranch aria-hidden="true" size={16} />Blast Radius</button></div><div className="mt-5" role="tabpanel">{mode === "path" ? <PathFinder services={services} /> : <BlastRadius services={services} />}</div></div>;
}
