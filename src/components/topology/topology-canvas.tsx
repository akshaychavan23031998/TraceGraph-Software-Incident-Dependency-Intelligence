...
function TopologyWorkspace({ topology }: { topology: ServiceTopology }) {
  ...
  const selectService = useCallback((service: TopologyNode) => { setSelectedId(service.id); setSearch(""); requestAnimationFrame(() => fitView({ nodes: [{ id: service.id }], duration: 450, maxZoom: 1.3, padding: 1.2 })); }, [fitView]);
